...
subplot(2,3,1)
axis([0 10 0 10]);
ca=[0 1 0.5;0 0 sqrt(3)/2];  %The coordinates of the points
[len,wid]=size(ca);
nor=ones(1,wid);
ha=[ca;nor];
...
daspect([1 1 1])
title('original equilateral triangle')
%% translation
subplot(2,3,2)
axis([0 10 0 10]);
ca=[0 1 0.5;0 0 sqrt(3)/2];  %The coordinates of the points
...
daspect([1 1 1])
title('triangle after translation')
ctrans=trans(len,wid);%Cartesian coordinates after translation
%% euclidean
subplot(2,3,3)
axis([0 10 0 10]);
ca=[0 1 0.5;0 0 sqrt(3)/2];  %The coordinates of the points
...
daspect([1 1 1])
title('triangle after euclidean')%translation (5,5) and rotation 30бу
...
daspect([1 1 1])
title('triangle after similarity')
...
pro=pro./pro(len+1,:);
for i=1:wid-1;
    %line([pro(1,i)/pro(3,i),pro(1,i+1)/pro(3,i+1)],[pro(2,i)/pro(3,i),pro(2,i+1)/pro(3,i+1)]);  %line([x1,x2],[y1,y2])
    line([pro(1,i),pro(1,i+1)],[pro(2,i),pro(2,i+1)]);
    hold on
end
hold on
line([pro(1,1),pro(1,wid)],[pro(2,1),pro(2,wid)]);
%line([pro(1,1)/pro(3,1),pro(1,wid)/pro(3,wid)],[pro(2,1)/pro(3,1),pro(2,wid)/pro(3,wid)]);  %line the first point and the last point
daspect([1 1 1])
title('triangle after projective')
